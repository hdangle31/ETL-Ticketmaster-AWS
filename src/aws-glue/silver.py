...
def flatten_event(df):
    ...
    df_flatten = df_flatten.withColumn("current_date", current_date)
...
s3_saving_path = f's3://{bucket_name}/dang3107/silver'

# Write to S3 silver bucket
flattened_df.write.mode('overwrite').partitionBy("current_date").parquet(s3_saving_path)
